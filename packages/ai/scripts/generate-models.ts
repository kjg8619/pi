...
async function loadModelsDevData(): Promise<Model<any>[]> {
	try {
		...
		// `kimi-code-plan-global` provider the reviewed pinned snapshot is emitted verbatim (no other provider
		// gains tolerance from this exception).
		const kimiCodingModels = data["kimi-code-plan-global"]?.models as Record<string, ModelsDevModel> | undefined;
		const liveKimiCodingModels = kimiCodingModels
			? Object.values(kimiCodingModels).filter((model) => (model as ModelsDevModel).tool_call === true).length
			: 0;
		...
		if (kimiCodingModels && !shouldUseKimiCodingFallback(liveKimiCodingModels)) {
			const kimiModels = kimiCodingModels;
