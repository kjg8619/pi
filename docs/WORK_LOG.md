...
| V0.4C Verifier Sandbox | 구현·자동 회귀 PASS / macOS actual PASS / 게시 `4909b10e7` | LOG-069. `verification.sandbox.mode`(기본 disabled), SRT 0.0.76 exact pin(company-runtime runtime dep; root/example 0.0.26 유지), SandboxPort(argv·canonical path·0600 temp settings), fixed deny-all network + workspace write allow + oracle/protected write·read deny, `sandboxPolicyDigest` Host-frozen Kernel guard. 자동 53개 파일·1,705개 PASS(macOS real sandbox fixtures 포함) |
