...
- **V0.6A/C07은 bounded 범위에서 CLOSED**다(LOG-101~105). Pi `fd0f93d58` exact CI PASS, T3 `09de732fe`는 main/PR-only trigger에 따른 exact SHA 미실행이다. **V0.6B/C06은 OPEN**이다. v1 실패와 LOG-111의 v2 oracle/usage 보강을 보존했고, LOG-112의 gate·전체 비교·usage detail 보강 `97ce0a2c6` exact CI PASS 뒤 새 허가의 actual calibration을 각각 한 번 실행했다. LOG-113에서 Astra F01 PASS/F02 BLOCKED·oracle FAIL, CommandCode F01 COMPLETED·oracle FAIL·falseCompletion=true를 기록했다. 새 실제 3 fixture의 usage KNOWN·cleanup CONFIRMED, 기존 T3의 v2/v1 분리·NOT COMPARABLE·UNKNOWN 화면, 직렬 최종 전체 회귀는 확인했다. Full actual F03–F10·V0.6C는 시작하지 않았다.
