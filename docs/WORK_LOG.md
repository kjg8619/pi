...
- **V0.5A·V0.5B·V0.5C·V0.5D는 현재 bounded 범위에서 CLOSED**다. D의 전체 로컬 회귀·Astra actual 1회·구현 및 별도 closure exact HEAD CI PASS는 LOG-086~093에 보존한다. V0.6A 첫 read-only module의 결과는 LOG-094~096이다. 두 번째 실제 **Weavra CLI → T3 backend → read-only frontend** 연결·화면·단절·fresh canonical 재연결, 양쪽 로컬 gate와 devlop 게시를 확인했다(LOG-097~100). Weavra 구현 exact CI는 PASS, T3 exact SHA CI는 workflow 조건에 따른 미실행이다. **두 번째 read-only slice는 완료이며 전체 C07/control은 OPEN**이다.
- V0.6A **C07 control의 로컬 acceptance는 PASS, 게시·exact CI는 대기**다(LOG-101~103). legacy read-only와 분리한 opt-in control, 공유 prepare/confirm·기존 cancel/R3 경로를 실제 T3 화면에서 검증했다. COMPLETE·cancel/reconnect·R3 CONSUMED/COMPLETED·DENIED/BLOCKED와 adversarial proof, 양쪽 전체 로컬 회귀를 확보했다. 최종 원격 판정 전이므로 전체 C07은 아직 OPEN이고 V0.6B는 미착수다.
