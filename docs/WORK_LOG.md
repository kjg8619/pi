...
- **V0.6C/C08 전체는 OPEN**이며 첫 read-only 수집은 구현·actual 검증했다(LOG-117). Pinned Jev reader + owned Chromium/profile + local static document → `CANDIDATE_ONLY`이며 check/Run/Kernel authority를 만들지 않는다. Node 24 actual boundary/cleanup과 Node 26 escaped projection, admission 7 tests 및 최종 Pi 전체 216.57초 exit 0을 확인했다. 정상 commit/push·exact CI 게시 확인은 다음 gate다.
