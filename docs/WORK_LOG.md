...
- **V0.6A/C07은 bounded 범위에서 CLOSED**다(LOG-101~105). Pi `fd0f93d58` exact CI PASS, T3 `09de732fe`는 main/PR-only trigger에 따른 exact SHA 미실행이다. **V0.6B/C06은 bounded 구현·faux proof·T3 read-only 비교 후 두 actual calibration 실패로 OPEN**이다. Astra F02·CommandCode F01에서 멈췄고 full matrix·V0.6C는 시작하지 않았다. LOG-111의 후속 재감사는 oracle/usage 경계를 수정한 corpus v2를 faux로 검증했으며 기존 v1 actual 기록은 보존했다. 현재 근거는 LOG-107~111이고 C07의 과거 proof와 구분한다.
