...
- **V0.5A·V0.5B·V0.5C·V0.5D는 현재 bounded 범위에서 CLOSED**다. D의 전체 로컬 회귀·Astra actual 1회·구현 및 별도 closure exact HEAD CI PASS는 LOG-086~093에 기록했다. 그 뒤 V0.6A 첫 read-only query/event/canonical reconnect를 실제 Runtime/SDK + in-memory faux JSONL로 구현했으며 smoke·전체 로컬/static gate도 PASS다(LOG-094~095). 전체 C07/T3 UI/control 완료가 아니며 게시 CI는 GitHub Actions의 정확한 commit HEAD 실행으로 구분한다.
