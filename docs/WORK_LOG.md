...
- **V0.6A/C07은 bounded 범위에서 CLOSED**다(LOG-101~105). **V0.6B/C06은 full actual·최종 로컬/UI proof 완료, closure evidence 게시·exact CI 대기**다(LOG-114~115). 독립 audit 후 v3 source `b2b938aa2`를 게시하고 exact CI PASS 뒤 Astra/DeepSeek fresh full을 각각 한 번 실행했다. 두 record 모두 10 fixture 수집·CALIBRATION_READY/EVALUATION_COMPLETE·oracle PASS 10·falseCompletion 0이다. F06 BLOCKED, strict receipt 1/3, F08 UNKNOWN usage와 historical v1/v2 bytes/원인 UNKNOWN을 보존했다. Actual read-only T3와 최종 Pi 전체 218.37초 exit 0을 확인했다. 마지막 evidence 게시 gate 전까지 C08/Jev는 미착수다.
- 안정 기준: `weavra-v0.1-rc1`은 immutable historical RC baseline이며 이동하지 않는다.
- 아직 NOT VERIFIED: **Linux verifier sandbox actual**(runner 외부 blocker 2건 — V0.4C는 macOS actual PASS로 CLOSED), TUI overlay rendering, V0.4A strict의 R2/R3·QUICK actual smoke, Plain Pi vs Weavra 실제 반복 비교, 20 fixture corpus 확대, R2/R3 measurement/evidence actual Provider smoke, 추가 모델/OS/Node matrix, COMPLEX/parallel, browser/MCP/memory, external TOCTOU의 완전 해소(주장하지 않음). V0.5C actual은 통제 defect 1회이며 R2/R3 repair·expected-failure TDD·일반 retry/resume는 미지원이다. V0.5D 착수 HEAD는 `1d2a4af96`, 구현 `0d4e91936`의 [CI 35454385916](https://github.com/kjg8619/pi/actions/runs/35454385916) 및 closure `9331b3ed9`의 [CI 35454820292](https://github.com/kjg8619/pi/actions/runs/35454820292)는 success다. V0.6A의 T3 read-only UI는 LOG-097에서 실제 관찰했으나 Host owner liveness/cleanup, 실행 control, 별도 network service/replay는 제공하지 않는다.
