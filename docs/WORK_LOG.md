...
- **V0.6A/C07은 bounded 범위에서 CLOSED**다(LOG-101~105). Pi `fd0f93d58` exact CI PASS, T3 `09de732fe`는 main/PR-only trigger에 따른 exact SHA 미실행이다. **V0.6B/C06은 bounded corpus·CLI·immutable result·faux proof를 구현했고 actual calibration·최종 CI 전이므로 OPEN**이다(LOG-107~108). 과거 C07 paid-provider/다른 OS 검증으로 확대하지 않는다.
