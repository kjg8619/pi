...
- **V0.6A/C07은 bounded 범위에서 CLOSED**다(LOG-101~105). Pi `fd0f93d58` exact CI PASS, T3 `09de732fe`는 main/PR-only trigger에 따른 exact SHA 미실행이다. **V0.6B/C06은 OPEN**이다. v1 Astra F02·CommandCode F01 실패와 LOG-111의 v2 oracle/usage 보강은 보존한다. LOG-112는 새 actual 허가 후 calibration gate·전체 corpus 비교 조건·usage detail provenance를 보강하고 로컬 전체 회귀를 통과한 단계다. clean committed harness의 v2 실제 F01/F02를 각각 한 번 실행하기 전이며, 두 target이 모두 통과하기 전에는 full matrix·V0.6C를 시작하지 않는다.
