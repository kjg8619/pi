...
- **V0.5A·V0.5B·V0.5C는 CLOSED**다. V0.5C의 `codex-lb/gpt-6-astra` actual repair·strict mutation/trust·macOS required sandbox 결과와 당시 full regression은 LOG-084~085에 보존한다. **V0.5D C04/C05는 구현·회귀 진행 중**이며 LOG-086~088의 현재 결과와 과거 결과를 구분한다. V0.5D actual/전체 회귀/exact remote CI/별도 closure는 아직 미완료이고 V0.6A는 시작하지 않았다.
