...
- **V0.5A·V0.5B·V0.5C는 CLOSED**다. V0.5C의 actual repair·당시 full regression은 LOG-084~085에 보존한다. **V0.5D C04/C05는 전체 로컬 회귀와 Astra actual 1회 PASS, remote closure 대기**다(LOG-086~091). fresh impact/docs·독립 review·strict mutation/trust·macOS required sandbox·oracle PASS·COMPLETED·writer release를 확인했다. 정확한 구현 HEAD CI와 별도 closure docs HEAD CI는 아직 미완료이며 V0.6A는 시작하지 않았다.
