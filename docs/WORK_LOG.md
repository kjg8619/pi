...
- **V0.5A·V0.5B·V0.5C·V0.5D는 현재 bounded 범위에서 CLOSED**다. D의 전체 로컬 회귀·Astra actual 1회·구현 및 별도 closure exact HEAD CI PASS는 LOG-086~093에 기록했다. 그 뒤 V0.6A 첫 read-only query/event/canonical reconnect를 실제 Runtime/SDK + in-memory faux JSONL로 구현했으며 smoke·전체 로컬/static gate·구현 `bacbb778e`의 exact CI도 PASS다(LOG-094~096). **첫 read-only slice는 완료, 전체 C07/T3 UI/control은 OPEN**이다. 마지막 검증 기록 문서의 CI는 별도 exact HEAD 게시 gate로 확인한다.
- 안정 기준: `weavra-v0.1-rc1`은 immutable historical RC baseline이며 이동하지 않는다.
- 아직 NOT VERIFIED: **Linux verifier sandbox actual**(runner 외부 blocker 2건 — V0.4C는 macOS actual PASS로 CLOSED), TUI overlay rendering, V0.4A strict의 R2/R3·QUICK actual smoke, Plain Pi vs Weavra 실제 반복 비교, 20 fixture corpus 확대, R2/R3 measurement/evidence actual Provider smoke, 추가 모델/OS/Node matrix, COMPLEX/parallel, browser/MCP/memory, external TOCTOU의 완전 해소(주장하지 않음). V0.5C actual은 통제 defect 1회이며 R2/R3 repair·expected-failure TDD·일반 retry/resume는 미지원이다. V0.5D 착수 HEAD는 `1d2a4af96`, 구현 `0d4e91936`의 [CI 35454385916](https://github.com/kjg8619/pi/actions/runs/35454385916) 및 closure `9331b3ed9`의 [CI 35454820292](https://github.com/kjg8619/pi/actions/runs/35454820292)는 success다. V0.6A는 durable observation이며 Host owner liveness/cleanup, T3 UI/control/network service/replay는 제공하지 않는다.
