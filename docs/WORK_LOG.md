...
- **V0.6C/C08 전체는 OPEN**이며 첫 read-only 수집은 구현·actual 검증·정상 게시했다(LOG-117~118). Pinned Jev reader + owned Chromium/profile + local static document → `CANDIDATE_ONLY`이며 check/Run/Kernel authority를 만들지 않는다. Node 24 actual boundary/cleanup과 Node 26 escaped projection, admission 7 tests 및 Pi 전체 216.57초 exit 0 뒤 source `81b4435d26373617116eac17d25f106f7f830889`의 [exact CI 35521294286](https://github.com/kjg8619/pi/actions/runs/35521294286)도 PASS했다. Live browser 독립 Verifier와 행동 정책은 별도 후속이다.
- 안정 기준: `weavra-v0.1-rc1`은 immutable historical RC baseline이며 이동하지 않는다.
- 아직 NOT VERIFIED: **Linux verifier sandbox actual**(runner 외부 blocker 2건 — V0.4C는 macOS actual PASS로 CLOSED), TUI overlay rendering, V0.4A strict의 R2/R3·QUICK actual smoke, Plain Pi vs Weavra 실제 반복 비교, 20 fixture corpus 확대, R2/R3 measurement/evidence actual Provider smoke, 추가 모델/OS/Node matrix, COMPLEX/parallel, live browser action/독립 browser Verifier/MCP/memory, external TOCTOU의 완전 해소(주장하지 않음). V0.5C actual은 통제 defect 1회이며 R2/R3 repair·expected-failure TDD·일반 retry/resume는 미지원이다. V0.5D 착수 HEAD는 `1d2a4af96`, 구현 `0d4e91936`의 [CI 35454385916](https://github.com/kjg8619/pi/actions/runs/35454385916) 및 closure `9331b3ed9`의 [CI 35454820292](https://github.com/kjg8619/pi/actions/runs/35454820292)는 success다. V0.6A의 T3 read-only UI는 LOG-097에서 실제 관찰했으나 Host owner liveness/cleanup, 실행 control, 별도 network service/replay는 제공하지 않는다.
