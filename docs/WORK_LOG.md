...
- **V0.5B는 closure HEAD `a5023eb98`의 CI PASS까지 확인해 CLOSED**다. V0.5C는 sandbox target 종료 경계(`71ea7b2aa`, CI PASS)를 먼저 보강한 뒤, opt-in STANDARD/EDIT/R1·SELF_CHECK 최대 1회의 linked repair 첫 vertical slice를 구현했다(LOG-082). 실제 SDK/faux·macOS required sandbox·production command smoke·full local 회귀 PASS이며 새 구현 commit/push·해당 HEAD CI 확인이 남아 있다.
- 안정 기준: `weavra-v0.1-rc1`은 immutable historical RC baseline이며 이동하지 않는다.
- 아직 NOT VERIFIED: **Linux verifier sandbox actual**(runner 외부 blocker 2건 — V0.4C는 macOS actual PASS로 CLOSED), V0.5C 유료 Provider actual·TUI overlay rendering, V0.4A strict의 R2/R3·QUICK actual smoke, Plain Pi vs Weavra 실제 반복 비교, 20 fixture corpus 확대, R2/R3 measurement/evidence actual Provider smoke, 다른 OS/Node matrix, COMPLEX/parallel, browser/MCP/memory, external TOCTOU의 완전 해소(주장하지 않음). 최근 게시된 선행 경계 HEAD `71ea7b2aa`의 [CI 35443556517](https://github.com/kjg8619/pi/actions/runs/35443556517)은 success다. 이전 SHA의 CI를 새 구현/closure HEAD의 결과로 대체하지 않는다.
