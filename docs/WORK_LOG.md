...
- **V0.5A·V0.5B·V0.5C·V0.5D는 현재 bounded 범위에서 CLOSED**다. D의 전체 로컬 회귀·Astra actual 1회·구현 exact HEAD CI PASS와 closure 판정은 LOG-086~092에 기록했다. fresh impact/docs·독립 review·strict mutation/trust·macOS required sandbox·oracle PASS·COMPLETED·writer release를 확인했다. 별도 closure docs HEAD CI는 게시 gate로 남아 있고, 그 결과를 확인하기 전에는 V0.6A를 시작하지 않는다.
- 안정 기준: `weavra-v0.1-rc1`은 immutable historical RC baseline이며 이동하지 않는다.
- 아직 NOT VERIFIED: **Linux verifier sandbox actual**(runner 외부 blocker 2건 — V0.4C는 macOS actual PASS로 CLOSED), TUI overlay rendering, V0.4A strict의 R2/R3·QUICK actual smoke, Plain Pi vs Weavra 실제 반복 비교, 20 fixture corpus 확대, R2/R3 measurement/evidence actual Provider smoke, 추가 모델/OS/Node matrix, COMPLEX/parallel, browser/MCP/memory, external TOCTOU의 완전 해소(주장하지 않음). V0.5C actual은 통제 defect 1회이며 R2/R3 repair·expected-failure TDD·일반 retry/resume는 미지원이다. V0.5D 착수 HEAD는 `1d2a4af96`이고 구현 HEAD `0d4e91936`의 [CI 35454385916](https://github.com/kjg8619/pi/actions/runs/35454385916)는 success다. 별도 closure HEAD CI는 다음 gate다.
