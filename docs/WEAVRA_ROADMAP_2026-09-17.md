...
| **V0.6A — T3 Code Host Bridge** | 구조화 명령·이벤트·승인·취소·재연결 Host 계약 | C07 | **첫 Host module + 두 번째 실제 read-only vertical slice 완료**: 첫 slice는 LOG-094~096의 Runtime/SDK+faux·로컬 회귀·exact CI 기록, 두 번째는 Weavra CLI → T3 backend → Project Settings UI·snapshots-only 관찰/재연결. **전체 C07은 OPEN**: start/resume/cancel/approve/reject/write/edit control 미구현; 새 network service 없음 |
...
UI는 **Settings → project scope → Project → Weavra**의 overview·source graph node/edge·bounded evidence/config summary이며 grouped project는 environment/checkout 하나를 선택한다. current config와 frozen Run config, durable Run과 connection/owner를 구분한다. CONNECTED여도 owner UNKNOWN이고 writer presence는 liveness가 아니다. raw prompt/reasoning/credential/transcript/source/docs/tool output과 control button은 없다.
