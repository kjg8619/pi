...
| **V0.6C — Jev Browser Evidence** | 격리된 브라우저 탐색을 regression evidence 후보로 연결 | C08 | **OPEN, 첫 read-only slice 게시/검증**(LOG-117~118). C06 CLOSED 뒤 pinned Jev reader·owned profile·local static candidate를 구현했다. Source `81b4435d2` [exact CI 35521294286](https://github.com/kjg8619/pi/actions/runs/35521294286) PASS. Host-reviewed 등록·fresh 독립 browser Verifier·행동 정책과 전체 C08 closure는 별도 |
...
- 닫힌 command는 `control.hello`, `control.snapshot`, `workflow.prepare`, `workflow.confirm`, `workflow.cancel`, `approval.resolve` 여섯 개다. 별도 control request는 newline 포함 32 KiB, response는 64 KiB다. owner UUID + Runtime-issued 단조 증가 request ID + 최대 64개의 payload-bound receipt를 사용하며 같은 epoch의 evicted ID도 replay하지 않는다.
...
**C08 현재 범위:** [고정 upstream 조사와 첫 read-only slice](WEAVRA_AGENT_LANDSCAPE_AND_ADOPTION_2026-09-18.md#10-c08-후속-연구와-첫-read-only-slice-2026-09-21)를 구현·actual 검증했다. `weavra browser observe`는 새 private Chromium/profile·명시적 로컬 정적 문서·고정 Jev reader만 사용하고 `CANDIDATE_ONLY`를 반환한다. 앱 JavaScript/추가 요청/행동/모델 호출·자동 check 등록·T3 직결은 없다. 새 문서 수집, script/resource/redirect/frame/closed-shadow 거부, 취소·cleanup과 verifier schema 비수락을 실제 CLI에서 확인했다. Browser 전체 OS network sandbox를 주장하거나 기존 required verifier의 deny-all을 완화하지 않는다. **C08 전체는 OPEN**이며 다음은 run/fixture/origin에 bound된 fresh 독립 browser verification과 별도 action policy 설계다.
