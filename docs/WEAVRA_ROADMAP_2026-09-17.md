...
| **V0.6B — Provider Fitness Matrix** | Provider/model/endpoint별 Weavra 계약 적합성의 재현 가능한 평가 | C06 | [독립 audit·v3 full actual](WEAVRA_PROVIDER_FITNESS_PLAN_2026-09-20.md#16-fresh-v3-actual-full-collection). 두 target 각각 10 fixture 수집·비교·actual UI·최종 전체 로컬 회귀 완료. Historical v1/v2와 UNKNOWN 보존. **Closure evidence 게시·exact CI 대기** |
...
**V0.6B/C06은 full actual·최종 로컬/UI proof를 마치고 closure evidence 게시·exact CI를 기다린다.** 독립 F01/F02 audit 후 Pi source `b2b938aa2b890ec482265638fe882ad1e3bebb32`의 exact CI PASS를 먼저 확인하고 v3 fresh cohort를 target별 한 번씩 실행했다. Astra `c032d3ec-05cb-42d0-bafe-827c19a31003`, DeepSeek `04b72270-eea6-48c2-9362-4aa57285799c`는 각각 10 oracle PASS, falseCompletion 0, CALIBRATION_READY/EVALUATION_COMPLETE다. F06 BLOCKED/adherence=false, strict receipt rejection 1/3, 마지막 F08 cancellation/usage UNKNOWN을 보존했다. Known tokens 163,249/328,863은 전체 사용량이 아니며 total/cost/HTTP attempts는 UNKNOWN이다. 같은 full 조건의 read-only 비교와 실제 T3 UI, 최종 Pi 전체 218.37초 exit 0을 확인했다. T3 source `6cced3c7f`의 전체 로컬 gate는 PASS지만 devlop exact CI는 NOT RUN이다. v1/v2 bytes와 과거 원인 UNKNOWN 판정은 유지한다. [현재 근거](WEAVRA_PROVIDER_FITNESS_PLAN_2026-09-20.md#16-fresh-v3-actual-full-collection)를 정상 게시하고 exact CI를 확인한 뒤 C06을 닫으며, 그 전에는 V0.6C/Jev에 진입하지 않는다.
