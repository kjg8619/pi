...
| **V0.6A — T3 Code Host Bridge** | 구조화 명령·이벤트·승인·취소·재연결 Host 계약 | C07 | **두 read-only slice 완료 + 별도 opt-in control 구현**. legacy argv/protocol v1 유지, Runtime-owned prepare/confirm/cancel/scoped approval 응답. 실제 T3 COMPLETE·cancel/reconnect·R3 approve/consume·reject 및 adversarial proof 확보. 최종 게시/remote gates와 전체 C07은 **OPEN**; 새 network Host 없음 |
| **V0.6B — Provider Fitness Matrix** | Provider/model/endpoint별 Weavra 계약 적합성의 재현 가능한 평가 | C06 | **미착수**. C07 closure 이후 조건부인 지속 평가 track의 첫 정식 milestone |
...
**C07 control은 별도 opt-in endpoint로 구현했으며, 최종 acceptance/remote gates와 전체 V0.6A/C07은 OPEN이다.** trusted `T3_WEAVRA_EXECUTABLE`은 실제 `<Weavra checkout>/packages/company-runtime/bin/weavra` 절대경로이고, T3 server의 **`T3_WEAVRA_CONTROL=1`**에서만 `bridge --stdio --project-trusted --control`을 사용한다. legacy read-only argv/protocol v1은 그대로다.
...
**현재 actual proof와 남은 gate:** 실제 launcher → T3 backend → Project Settings UI와 로컬 faux Provider에서 edited AC preview 갱신·modal 확인 전 model 호출 0회, 실제 checks/독립 review 뒤 `COMPLETED`·active agents 0·writer 해제·파일 수정을 확인했다. blocked Developer의 browser blank/reconnect는 같은 RUNNING Run과 writer를 유지하고 명시적 cancel은 `CANCELLED`·active agents 0·writer 해제·faux SSE 종료·부분 변경 보존으로 끝났다. R3는 별도 승인 후 **CONSUMED → COMPLETED / 파일 삭제**, Reject 후 **DENIED → BLOCKED / 원본 보존**을 확인했다. 실제 wire의 replay/owner/root/framing/권한 위조와 browser raw frame의 credential/source sentinel 비노출도 확인했다. paid-provider 품질이나 다른 OS의 proof는 아니다.

현재 Pi full test/check/check:ci와 T3 typecheck/fmt/knip/build/lint exit 0을 확인했다(기존 무관한 warning 있음). 첫 T3 full test는 localhost `ETIMEDOUT` 1건으로 실패했고, package 동시 실행을 끈 전체 재실행은 **1,269 files / 17,262 PASS / 58 skipped**였다. 초기 실패는 이력에 보존하고 OS-level 원인 해결로 과장하지 않는다. 최종 게시와 양쪽 최종 HEAD/remote gates는 OPEN이며 앞의 read-only CI 결과를 현재 control SHA에 승계하지 않는다.

**V0.6B Provider Fitness Matrix는 C07 closure 이후 조건부이며 아직 착수하지 않았다.** Browser 확장도 별도 범위이며 자동 착수하지 않는다. control 상세는 [Runtime 계약](../packages/company-runtime/README.md#c07-별도-control-계약)을 따른다.
