...
| **V0.6B — Provider Fitness Matrix** | Provider/model/endpoint별 Weavra 계약 적합성의 재현 가능한 평가 | C06 | [bounded CLI/corpus/immutable raw results 및 actual 결과](WEAVRA_PROVIDER_FITNESS_PLAN_2026-09-20.md#10-실제-calibration과-중단-판정). faux proof·T3 read-only 비교 완료. Astra F02·CommandCode F01 calibration 실패로 full matrix 미실행, **OPEN**. 자동 fallback/종합 점수 없음 |
...
**V0.6B의 bounded Fitness CLI·immutable records·T3 read-only 비교는 구현했으나 C06은 OPEN이다.** 과거 v1 실패를 보존한 뒤, 새 허가에 따라 corpus v2·clean harness `97ce0a2c69cbd6605011fbe34a54b07c9ec11d29`의 [exact CI 35509228483](https://github.com/kjg8619/pi/actions/runs/35509228483) PASS와 committed targeted/faux/CLI gate를 확인하고 두 calibration을 각각 한 번 실행했다. Astra는 F01 PASS/F02 BLOCKED·oracle FAIL·Task Contract adherence=false, CommandCode는 F01 COMPLETED·oracle FAIL·falseCompletion=true여서 F02 미실행이다. 실제 3 fixture 모두 usage KNOWN·cleanup CONFIRMED이며 두 record는 CALIBRATION_FAILED다. 나머지 F03–F10, 재시도/diagnostic/fallback, oracle 완화와 V0.6C/Jev 착수는 하지 않았다. 기존 T3 화면에서 새 v2·과거 v1 분리, exact identity, raw counts/UNKNOWN, NOT COMPARABLE·fixtures differs를 확인했다. 상세 raw dimensions·한계·검증·게시 상태는 [v2 actual 기록](WEAVRA_PROVIDER_FITNESS_PLAN_2026-09-20.md#14-새-허가에-따른-v2-actual-calibration--c06-open)과 LOG-112~113을 따른다. C06 CLOSED 이후에만 V0.6C 조사·계획을 진행한다.
