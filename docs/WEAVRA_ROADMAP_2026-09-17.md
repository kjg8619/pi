...
**V0.6B의 bounded Fitness CLI·immutable records·T3 read-only 비교는 구현했으나 C06은 OPEN이다.** clean committed harness의 actual calibration에서 Astra F02와 CommandCode F01이 실패하여 full matrix를 실행하지 않았다. 결과를 구제하는 재시도·모델 대체·oracle 완화는 없다. C06 CLOSED 이후에만 V0.6C 조사·계획을 진행한다. 실제 결과와 검증·게시 상태는 [Fitness 기록](WEAVRA_PROVIDER_FITNESS_PLAN_2026-09-20.md#10-실제-calibration과-중단-판정)과 WORK_LOG를 따른다.
