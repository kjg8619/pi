...
| **V0.6C — Jev Browser Evidence** | 격리된 관찰을 Host-reviewed regression check와 fresh evidence로 연결 | C08 | **OPEN, bounded 구현·전체 로컬 검증 완료/게시 대기**(LOG-119~120). Candidate v2 → Host preview/confirm → 기존 registry/RegisteredVerifier → fresh SELF_CHECK/TEST → Kernel을 연결했다. 실제 Chromium·SDK faux workflow·T3 등록 UI와 Pi/T3 전체 회귀를 검증했다. 정상 source/docs 게시와 exact CI 후 bounded closure를 판정하며 action policy는 후속 범위다 |
...
**C08 현재 범위:** [첫 read-only slice](WEAVRA_AGENT_LANDSCAPE_AND_ADOPTION_2026-09-18.md#10-c08-후속-연구와-첫-read-only-slice-2026-09-21)에 이어 [Host 등록과 fresh 독립 검증](WEAVRA_AGENT_LANDSCAPE_AND_ADOPTION_2026-09-18.md#11-c08-host-등록과-fresh-독립-검증-2026-09-21)을 구현했다. Schema 2 candidate는 여전히 `CANDIDATE_ONLY`이며 Host가 기대값을 수정하고 Runtime preview를 명시적으로 확인해야 기존 project registry에 등록된다. Frozen check/helper/executable identity 아래 매 SELF_CHECK/TEST마다 새 private HOME/profile/CDP-pipe capture를 수행하고 Kernel이 typed evidence와 기존 revision/trust/review guards를 함께 확인한다. 실제 historical Ready/fresh Broken은 FAIL이며 완료를 막는다. Browser 실패는 자동 repair로 가지 않는다. 실제 Chromium·SDK faux whole workflow·T3 UI와 양쪽 전체 로컬 gate는 PASS, 정상 게시/exact CI 전이므로 **전체 closure는 OPEN**이다. Actions·auth/session 재사용·remote navigation/crawling·임의 JS는 미지원이며 browser 전체 OS sandbox나 기존 required deny-all 완화를 주장하지 않는다.
