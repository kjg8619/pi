...
| **V0.6B — Provider Fitness Matrix** | Provider/model/endpoint별 Weavra 계약 적합성의 재현 가능한 평가 | C06 | [독립 F01/F02 audit와 v3 계약](WEAVRA_PROVIDER_FITNESS_PLAN_2026-09-20.md#15-독립-f01f02-audit와-v3-결정). Historical v1/v2 보존·원인 UNKNOWN 정정, integrity/capability 분리 구현. 새 actual 전 전체 gate·게시·exact CI 진행 중, **OPEN**. 자동 fallback/종합 점수 없음 |
...
**V0.6B/C06은 OPEN이며 독립 audit 뒤 새 actual 선행 gate를 진행 중이다.** v1/v2 record bytes·digests는 보존한다. F01의 v2 oracle은 동등 표현을 FAIL하고 잘못된 경계/인과/결론·keyword 삽입을 PASS하는 반례를 확인했다. 과거 DeepSeek 제출 원문은 없으므로 정답 여부는 INSUFFICIENT_EVIDENCE/UNKNOWN이다. F02 exact-byte 과제는 유효하지만 QUICK Task Contract digest guard 순서와 R1 known_risks 지침 결함을 고쳤다. 과거 Astra의 adherence=false는 terminal acceptance 부재에서도 만들어져 정확한 차단 원인은 UNKNOWN으로 정정한다. 새 corpus v3는 명시적 bounded JSON 조사 계약, record schema v2는 별도 integrity/calibration/evaluation과 bounded audit를 사용한다. Semantic/contract FAIL·falseCompletion은 보존하고 안전하면 다음 fixture를 실행하며, protocol/측정/transport/auth/oracle/cleanup/usage 불확실성은 중단한다. C06 closure는 모델 전부 PASS가 아니라 두 target의 신뢰 가능한 full actual 수집·read-only 비교·UI·최종 검증/게시로 판단한다. 현재 새 실제 호출은 0회이며 source commit/push·exact CI 전에 시작하지 않는다. [현재 계약과 audit](WEAVRA_PROVIDER_FITNESS_PLAN_2026-09-20.md#15-독립-f01f02-audit와-v3-결정)을 따르며 C06 CLOSED 이후에만 V0.6C/Jev에 진입한다.
