...
| **V0.6A — T3 Code Host Bridge** | 구조화 명령·이벤트·승인·취소·재연결 Host 계약 | C07 | **첫 read-only slice 완료**(LOG-094~096): version 1 query/response·관측 event·canonical reconnect, real Runtime/SDK+faux smoke·전체 로컬 회귀·구현 exact CI PASS. **전체 C07은 OPEN**: T3 UI·run/write/start/approval/cancel control·network service 미구현 |
...
**V0.6A 첫 연결 완료:** D 구현 `0d4e91936`의 [CI](https://github.com/kjg8619/pi/actions/runs/35454385916)와 별도 closure `9331b3ed9`의 [CI](https://github.com/kjg8619/pi/actions/runs/35454820292)가 모두 PASS인 것을 확인한 뒤 진행했다. repository-level read-only Host module + bounded local JSONL, 관측 event, fresh canonical snapshot reconnect를 실제 Runtime/SDK·in-memory faux로 검증했고, 전체 로컬 gate와 구현 `bacbb778e`의 [exact CI](https://github.com/kjg8619/pi/actions/runs/35457968130)도 PASS다. 검증 기록 문서 commit의 exact CI까지 확인한 뒤 중단한다. durable source와 owner-local 상태를 구분하고 Runtime/Kernel 단독 authority를 유지한다. 다음 범위는 T3 Host의 transport/owner-state 계약과 read-only 소비 경계를 별도로 결정하는 것이며 UI·run/write/start/approval/cancel control·Provider Matrix·Browser를 자동 착수하지 않는다.
