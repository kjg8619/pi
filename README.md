...
- V0.5B: reviewed recipe 4종(`bugfix`, `safe-refactor`, `test-addition`, `read-only-investigation`). recipe는 **Planner가 아니고 permission·approval·evidence·skill runtime·completion authority도 아니다**. 입력은 JSON editor로 받는 data일 뿐이며, 초안은 기존 AC 편집기에 prefill되고 사용자가 수정한 최종 AC만 Task Contract로 freeze된다. scope/check는 Host 소유(`allowed_paths`, required registered check)이고 recipe는 좁히기만 한다. provenance에는 `recipe{id,version,digest}`만 남고 recipe digest와 Task Contract digest는 별개다. 자동 discovery/install/hook/arbitrary shell 실행은 없으며 expected-failure TDD lifecycle은 구현하지 않았다.
...
| `/workflow run --recipe <id> <goal>` | reviewed recipe로 AC 초안을 만들어 기존 편집기/확인 단계에 넘긴다 |
